...
Skip requires a macOS 15+ development machine.
:::

<Steps>
1. Install [Xcode](https://developer.apple.com/xcode)
1. Open Xcode and install iOS platform support (this will be offered at first launch)
...
1. Once your app pre-builds and opens in Xcode, you are _almost_ ready to go. One more step is needed, which is to create and launch an Android emulator (which is the Android equivalent of the iPhone Simulator and is used for local app development). The `skip` tool has a command to quickly install and configure an Android emulator for your project:
   ...
   Once this command completes successfully, you will have an Android emulator installed, which you can launch and observe the logging output with:
    ...
    :::tip
    You can alternatively configure and launch an emulator using a graphical user interface by running Android Studio and opening the `Virtual Device Manager` from the ellipsis menu of the Welcome dialog. From there, `Create Device` (e.g., "Pixel 6") and then `Launch` the emulator. See [Create and manage virtual devices](https://developer.android.com/studio/run/managing-avds).
    :::
