...
[<a href="#v" onclick="jumpToTime('0:00')">0:00</a>] Introducing Skip, a new technology for iOS and Android mobile app development.
Let’s dive right in and hit the run button in Xcode.

[<a href="#v" onclick="jumpToTime('0:15')">0:15</a>] We’re now running the Skip sample Weather app.
...
[<a href="#v" onclick="jumpToTime('0:48')">0:48</a>] And here is the SwiftUI ListView displaying this list of cities.
Tapping a city pushes the WeatherView to display that city’s current weather.

[<a href="#v" onclick="jumpToTime('1:12')">1:12</a>] The WeatherView uses an observable WeatherCondition to fetch the latest weather.
WeatherCondition has observable properties, and it uses an async function to retrieve the weather.

[<a href="#v" onclick="jumpToTime('1:33')">1:33</a>] So that was a whirlwind tour of some of our iOS code.
...
[<a href="#v" onclick="jumpToTime('1:52')">1:52</a>] Let’s prove it. When the WeatherView displays the temperature, let’s have it display a message too.
And let’s ask Xcode to build and run…

[<a href="#v" onclick="jumpToTime('2:40')">2:40</a>] And there’s our new addition to the weather view, running on both platforms.
...
[<a href="#v" onclick="jumpToTime('3:02')">3:02</a>] Skip has a surprise for us here as well.
...
[<a href="#v" onclick="jumpToTime('3:17')">3:17</a>] It’ll take a minute to recompile everything for Mac, but we think it’s worth showing off. 
...
[<a href="#v" onclick="jumpToTime('3:34')">3:34</a>] If we watch the output we can see that the tests are run for iOS first, and then skip launches Android’s build tool Gradle to test with Roboelectric.

[<a href="#v" onclick="jumpToTime('4:04')">4:04</a>] Now let’s introduce a failure case to see what happens.
...
[<a href="#v" onclick="jumpToTime('4:24')">4:24</a>] Now the failure has been noted for iOS. And the Android tests should run in a moment…
Now we see that the test failed on Android as well. We can see the details of both failures right here in Xcode.

[<a href="#v" onclick="jumpToTime('4:40')">4:40</a>] OK, so what is happening?
We have an iOS app, with iOS unit tests, using Xcode, and now it’s all running on Android.
It feels like we “skipped” something.

[<a href="#v" onclick="jumpToTime('4:54')">4:54</a>] Well, Skip is not somehow emulating iOS on Android.
Instead, let’s see what Skip is doing.

[<a href="#v" onclick="jumpToTime('5:02')">5:02</a>] Skip is actually a number of things.
One is that it’s a Swift Package Manager plugin - we’ve seen it in action already, running through Xcode.

[<a href="#v" onclick="jumpToTime('5:13')">5:13</a>] Another is that it’s an intelligent Swift to Kotlin transpiler. 
...
[<a href="#v" onclick="jumpToTime('5:33')">5:33</a>] We have the Swift code we wrote on the left, and here is the corresponding Kotlin that Skip generated.
...
[<a href="#v" onclick="jumpToTime('6:13')">6:13</a>] If you know Kotlin, most of this should look familiar.
But you might be wondering how Kotlin is using iOS Foundation API like URLSession and JSONDecoder.

[<a href="#v" onclick="jumpToTime('6:38')">6:38</a>] Skip is also a set of Android libraries. 
...
[<a href="#v" onclick="jumpToTime('7:02')">7:02</a>] And that assertion error we received when we created a failing unit test? That was courtesy of the SkipUnit library, which converts your XCTest cases into JUnit, which is the standard Java and Kotlin unit testing framework.

[<a href="#v" onclick="jumpToTime('7:19')">7:19</a>] Skip’s transpiler is proprietary, but all of these libraries are free and open source, and we welcome contributions.

[<a href="#v" onclick="jumpToTime('7:28')">7:28</a>] So Skip uses intelligent transpilation plus a set of open source libraries to create your Android app. 
What are the advantages of this approach to dual-platform development?

[<a href="#v" onclick="jumpToTime('7:40')">7:40</a>] First, the fact that Skip uses transpilation  means there is no hidden runtime environment on either platform.
...
[<a href="#v" onclick="jumpToTime('8:04')">8:04</a>] This sample Weather app distribution is just kilobytes in size on iOS.

[<a href="#v" onclick="jumpToTime('8:10')">8:10</a>] The transpiled Android app is also a fully native Kotlin and Compose app.
...
[<a href="#v" onclick="jumpToTime('8:31')">8:31</a>] That means fully native look and feel, and fully native performance on both platforms.
...
[<a href="#v" onclick="jumpToTime('8:59')">8:59</a>] In fact here I’ve opened the generated Android app right in Android Studio.
Here’s the transpilation of our Weather model again.

[<a href="#v" onclick="jumpToTime('9:15')">9:15</a>] So what’s the debugging story in Skip?
...
[<a href="#v" onclick="jumpToTime('9:38')">9:38</a>] But if it’s an Android-specific bug, you can run the app right in Android Studio and utilize the full power of its own debugging tools. You get the native debugging experience on both platforms.

[<a href="#v" onclick="jumpToTime('9:53')">9:53</a>] That includes being able to dig into Skip’s own source code.
...
[<a href="#v" onclick="jumpToTime('10:10')">10:10</a>] One interesting note is that VStack, like most of Skip’s library code, is actually written in Swift. What we’re seeing is its transpilation.
Let’s look at its Swift source code to see how we can call Kotlin and Compose API from Swift.

[<a href="#v" onclick="jumpToTime('10:32')">10:32</a>] So what probably jumps out at you right away is this #if SKIP block at the top.
...
[<a href="#v" onclick="jumpToTime('11:00')">11:00</a>] Skip has other mechanisms too, including being able to add pure Kotlin files to your Xcode project and have them automatically included in your Android build.

[<a href="#v" onclick="jumpToTime('11:10')">11:10</a>] So another advantage of Skip’s approach is that integrating with other Kotlin and Java and Compose code is natural. It doesn’t require any complicated bridging.

[<a href="#v" onclick="jumpToTime('11:22')">11:22</a>] So that is Skip.
We’ll end this with some SwiftUI, as only Skip can.

[<a href="#v" onclick="jumpToTime('11:31')">11:31</a>] With Skip, you create an iOS app using Swift and SwiftUI. You work in Xcode.
...
[<a href="#v" onclick="jumpToTime('11:55')">11:55</a>] Skip is new, but we’re moving fast.
